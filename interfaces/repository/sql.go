...
var sql = map[string]string{
	...
	`customerValid`:  `SELECT id FROM movies.customer WHERE login = $1`,
	`customerLogin`:  `SELECT id, login, name, age, phone, createat FROM movies.customer WHERE login=$1 AND pass=$2`,
	`customerInsert`: `INSERT INTO movies.customer (login, pass, name, age, phone, createat) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`,

	`rentValid`:  `SELECT createAt FROM movies.rent WHERE customerId = $1 AND movieId = $2 AND active = $3`,
	`rentInsert`: `INSERT INTO movies.rent (customerId, movieId, active, createAt) VALUES ($1, $2, $3, $4) RETURNING id`,
	`rentUpdate`: `UPDATE movies.rent SET active=$3, closeAt=$4 WHERE active = true AND customerId = $1 AND movieId = $2`,
	...
	`rentFrom`:   `FROM movies.rent r, movies.movie m, movies.movie_genre mg, movies.genre g WHERE r.movieId = m.id AND mg.movieId = m.id AND mg.genreId = g.id AND r.customerId = $3 AND r.active = $4 `,
